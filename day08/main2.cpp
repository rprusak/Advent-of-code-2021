...
-> brute force using a contains(s, t) function to test for the existence of s in t, sorting to
reason top-down by process of elimination, and sorting to normalize any signal occurrences hashed as
keys
...
auto analyzeSignal(std::vector<std::string>& knownSignals, const std::string& unknownSignal)
    ...
    if (contains(unknownSignal, eight)) {
        ...
        if (unknownSignal.size() == 6) {
            if (contains(seven, unknownSignal) && contains(one, unknownSignal) &&
                contains(four, unknownSignal)) {
                // std::cout << "parsed " << unknownSignal << " -> 9" << std::endl;
                knownSignals[9] = unknownSignal;
            } else if (contains(seven, unknownSignal) && contains(one, unknownSignal) &&
                       !contains(four, unknownSignal)) {
        ...
        // get the remaining 2, 3, 5
            ...
            if (contains(unknownSignal, nine) && !contains(unknownSignal, six) &&
                contains(seven, unknownSignal) && contains(one, unknownSignal) &&
                !contains(four, unknownSignal)) {
                // std::cout << "parsed " << unknownSignal << " -> 3" << std::endl;
                knownSignals[3] = unknownSignal;
            } else if (contains(unknownSignal, six) && !contains(unknownSignal, zero) &&
                       !contains(seven, unknownSignal) && !contains(one, unknownSignal) &&
                       !contains(four, unknownSignal)) {
...
auto generateSignalMappings(const std::vector<std::string>& knownPatterns)
    ...
    for (std::size_t valueIdx = 0; valueIdx < knownPatterns.size(); ++valueIdx) {
        // std::cout << "knownPattern: " << knownPatterns[valueIdx] << " -> " << valueIdx <<
        // std::endl;
...
auto splitPatternIntoKnownsAndUnknowns(const std::string& pattern)
    -> std::pair<std::vector<std::string>, std::vector<std::string>> {
    auto uniqueSizeToDigit =
        std::unordered_map<std::size_t, std::uint32_t>{{2, 1}, {3, 7}, {4, 4}, {7, 8}};
...
auto processSignalPatterns(const std::vector<std::string>& signalPatterns,
                           const std::vector<std::string>& outputPatterns) -> std::uint32_t {
